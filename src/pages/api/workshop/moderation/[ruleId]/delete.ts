...
import { deleteWorkshopModerationRule } from '@/server/config/service';
...
export const POST: APIRoute = async (context) => {
  ...
  const returnTo = sanitizeWorkshopNextPath(
    String(parsedForm.formData.get('returnTo') ?? '/workshop/moderation'),
  );
  const result = await deleteWorkshopModerationRule({
