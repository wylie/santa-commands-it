...
import { setWorkshopModerationRuleActive } from '@/server/config/service';
...
export const POST: APIRoute = async (context) => {
  ...
  const result = await setWorkshopModerationRuleActive({
  ...
  if (result.status === 'success') {
    const suffix = result.activityLogged ? 'enabled' : 'enabled-with-warning';
