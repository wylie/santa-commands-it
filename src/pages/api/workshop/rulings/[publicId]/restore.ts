...
import { restoreWorkshopRuling } from '@/server/workshop/service';
...
export const POST: APIRoute = async (context) => {
  ...
  const result = await restoreWorkshopRuling({
  ...
  const error =
    result.status === 'already-public' ? 'already-public' : 'not-found';
  return context.redirect(
